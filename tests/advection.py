...
ds["vertices"]["conn_v2e"] = [f"{mesh.v2e[i, :]}" for i in range(mesh.num_vertices)]
ds["vertices"]["conn_v2c"] = [f"{mesh.v2c[i, :]}" for i in range(mesh.num_vertices)]
...
def compute_edge_centers(mesh):
    ...
    edge_centers = (mesh.xyarc[mesh.e2v[pole_edge_mask, 0]]+mesh.xyarc[mesh.e2v[pole_edge_mask, 1]])/2
...
for cell_id in range(mesh.num_cells):
    ...
    for vertex_id in range(0, mesh.c2v.shape[1]):
        if mesh.c2v[cell_id, vertex_id] != -1:
            num_vertices_per_cell += 1
            cell_centers[cell_id, 0:2] += mesh.xyarc[mesh.c2v[cell_id, vertex_id], :]
...
ds["cell_centers"]["conn_c2v"] = [f"{mesh.c2v[i, :]}" for i in range(mesh.num_cells)]
ds["cell_centers"]["conn_c2e"] = [f"{mesh.c2e[i, :]}" for i in range(mesh.num_cells)]
...
e2v_neighbor_table = NeighborTableOffsetProvider(mesh.e2v,
                                                 Edge, Vertex, 2)
v2e_neighbor_table = NeighborTableOffsetProvider(mesh.v2e,
                                                 Vertex, Edge, mesh.v2e.shape[1])
offset_provider = {
    "E2V": e2v_neighbor_table,
    "V2E": v2e_neighbor_table
}
